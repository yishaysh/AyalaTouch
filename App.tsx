...
const App: React.FC = () => {
  ...
  const handleUpdateOrder = (items: OrderItem[]) => {
    setTables(prev => prev.map(t => {
      if (t.id === selectedTableId) {
        return { ...t, currentOrder: items, status: items.length > 0 ? TableStatus.OCCUPIED : TableStatus.FREE };
      }
      return t;
    }));
  };
  ...
  const handleCloseTableTrigger = () => {
    requestConfirmation(
        'סגירת חשבון',
        'האם לסגור את השולחן ולבצע צ׳ק אאוט?',
        () => {
            if (selectedTableId === null) return;
            setTables(prev => prev.map(t => {
            if (t.id === selectedTableId) {
                ...
                if (t.currentOrder.length > 0) {
                    const pastOrder: PastOrder = {
                        id: `hist_${Date.now()}`,
                        items: [...t.currentOrder],
                        total: t.currentOrder.reduce((acc, i) => acc + i.price, 0),
                        date: new Date()
                    };
  ...
  const handleResetTableTrigger = () => {
    requestConfirmation(
        'איפוס שולחן',
        'פעולה זו תמחק את ההזמנה הנוכחית ללא שמירה. האם להמשיך?',
        () => {
            if (selectedTableId === null) return;
            setTables(prev => prev.map(t => {
                if (t.id === selectedTableId) {
  ...
  const handleEndShiftTrigger = () => {
      requestConfirmation(
          'סיום משמרת',
          'פעולה זו תאפס את כל השולחנות הפעילים. האם להמשיך?',
          () => {
            setTables(prev => prev.map(t => {
                const newHistory = [...(t.orderHistory || [])];
                if (t.currentOrder.length > 0) {
  ...
  const fetchChecklist = async () => {
    ...
    if (!checklistContent) {
        ...
        const content = await getWaitStaffChecklist();
  ...
  const getTimerColor = (minutes: number) => {
      if (minutes > 20) return 'bg-red-100 border-red-300 text-red-800 animate-pulse';
      if (minutes > 10) return 'bg-yellow-100 border-yellow-300 text-yellow-800';
  ...
  const renderKitchen = () => (
     <div className="p-4 md:p-6 h-full overflow-y-auto pb-24 md:pb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tables.filter(t => t.status === TableStatus.ORDERED).length === 0 && (
                <p className="text-gray-500 text-lg">אין הזמנות ממתינות במטבח כרגע.</p>
            )}
            {tables.filter(t => t.status === TableStatus.ORDERED).map(t => {
                ...
                return (
                <div key={t.id} className={`border-l-4 rounded-xl shadow-sm p-4 ${colorClass}`}>
                    <div className="flex justify-between border-b pb-2 mb-2">
                        <span className="font-bold text-lg">{t.name}</span>
                        <div className="flex items-center gap-2">
                            <Clock size={14} />
                            <span className="font-mono text-sm font-bold">{elapsed} דק'</span>
                        </div>
                    </div>
                    <ul className="space-y-2 mb-4">
                        {t.currentOrder.map(item => (
                            <li key={item.uniqueId} className={`flex justify-between items-center text-sm p-2 rounded ${item.isUrgent ? 'bg-red-200 text-red-900 font-bold' : ''}`}>
                                <span>
                                    {item.name} 
                                    {item.isUrgent && <span className="mr-2 text-xs bg-red-600 text-white px-1 rounded">דחוף</span>}
                                </span>
                    ...
                    <button 
                        onClick={() => {
                            setTables(prev => prev.map(table => 
                                table.id === t.id ? {...table, status: TableStatus.PAYMENT} : table
                            ))
                            showToast(`שולחן ${t.name} סומן כמוכן`);
                        }}
                        className="w-full py-2 bg-white/50 hover:bg-emerald-100 hover:text-emerald-700 border border-transparent hover:border-emerald-200 rounded-lg font-bold text-sm transition-colors flex justify-center items-center gap-2"
                    >
                        <CheckCircle size={16} />
                        סמן כמוכן
                    </button>
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 flex-col md:flex-row overflow-hidden">
      ...
      <main className="flex-1 relative overflow-hidden h-full order-1 md:order-2 bg-slate-50 pt-10 md:pt-0 pb-16 md:pb-0">
        ...
        {activeView === 'kitchen' && renderKitchen()}
                ...
                <div className="bg-white p-8 rounded-2xl w-full max-w-md shadow-2xl relative animate-in fade-in zoom-in">
                    ...
                    <form onSubmit={handleAdminLogin} className="space-y-4">
                        <input 
                            ...
                            pattern="[0-9]*"
                            maxLength={4}
                            className="w-full text-center text-4xl tracking-[1em] font-bold p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary outline-none"
                            value={adminPin}
                            onChange={(e) => setAdminPin(e.target.value)}
                            autoFocus
                        />
                        {loginError && <p className="text-red-500 text-center font-bold">קוד שגוי, נסה שנית</p>}
                        <button type="submit" className="w-full bg-secondary text-white py-4 rounded-xl font-bold text-lg hover:bg-slate-800 transition-colors">
                            כנס למערכת
                        </button>
                    </form>
                </div>
        ...
        {showChecklist && (
            <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
                <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col shadow-2xl">
                    <div className="p-4 border-b flex justify-between items-center bg-secondary text-white">
                        <h3 className="font-bold text-xl">רשימת דרישות להקמת האפליקציה</h3>
                        <button onClick={() => setShowChecklist(false)} className="hover:bg-white/20 p-2 rounded-full"><X /></button>
                    </div>
        ...
        <PrintModal 
            ...
            htmlContent={printModal.htmlContent}
            onConfirm={() => {
                printModal.onConfirm();
